...
namespace lsp
{
    ...
    namespace meta
    {
        ...
        static const meta::port_t config_metadata[] =
        {
            ...
            INT_CONTROL_ALL(UI_DLG_SAMPLE_FTYPE_ID, "Dialog file type for selecting sample files", U_NONE, 0, 100, 0, 1),
            PATH(UI_DLG_IR_PATH_ID, "Dialog path for selecting impulse response files"),
            INT_CONTROL_ALL(UI_DLG_IR_FTYPE_ID, "Dialog file type for selecting impulse response files", U_NONE, 0, 100, 0, 1),
            PATH(UI_DLG_CONFIG_PATH_ID, "Dialog path for saving/loading configuration files"),
            INT_CONTROL_ALL(UI_DLG_CONFIG_FTYPE_ID, "Dialog file type for saving/loading configuration files", U_NONE, 0, 100, 0, 1),
            PATH(UI_DLG_REW_PATH_ID, "Dialog path for importing REW settings files"),
            INT_CONTROL_ALL(UI_DLG_REW_FTYPE_ID, "Dialog file type for importing REW settings files", U_NONE, 0, 100, 0, 1),
            PATH(UI_DLG_HYDROGEN_PATH_ID, "Dialog path for importing Hydrogen drumkit files"),
            INT_CONTROL_ALL(UI_DLG_HYDROGEN_FTYPE_ID, "Dialog file type for importing Hydrogen drumkit files", U_NONE, 0, 100, 0, 1),
            PATH(UI_DLG_LSPC_BUNDLE_PATH_ID, "Dialog path for exporting/importing LSPC bundles"),
            INT_CONTROL_ALL(UI_DLG_LSPC_BUNDLE_FTYPE_ID, "Dialog file type for exporting/importing LSPC bundles", U_NONE, 0, 100, 0, 1),
            PATH(UI_DLG_SFZ_PATH_ID, "Dialog path for exporting/importing SFZ files"),
            INT_CONTROL_ALL(UI_DLG_SFZ_FTYPE_ID, "Dialog file type for exporting/importing SFZ files", U_NONE, 0, 100, 0, 1),
            PATH(UI_DLG_MODEL3D_PATH_ID, "Dialog for saving/loading 3D model files"),
            INT_CONTROL_ALL(UI_DLG_MODEL3D_FTYPE_ID, "Dialog file type for saving/loading 3D model files", U_NONE, 0, 100, 0, 1),
            PATH(UI_DLG_DEFAULT_PATH_ID, "Dialog default path for other files"),
            INT_CONTROL_ALL(UI_DLG_DEFAULT_FTYPE_ID, "Dialog default file type for other files", U_NONE, 0, 100, 0, 1),
            ...
            SWITCH(UI_REL_PATHS_PORT_ID, NULL, "Use relative paths when exporting configuration file", 0.0f),
            KNOB(UI_SCALING_PORT_ID, "Manual UI scaling factor", U_PERCENT, 25.0f, 400.0f, 100.0f, 1.0f),
            SWITCH(UI_SCALING_HOST_PORT_ID, NULL, "Prefer host-reported UI scale factor", 1.0f),
            ...
            SWITCH(UI_PREVIEW_AUTO_PLAY_ID, NULL, "Enable automatic playback of the audio file in the file preview part of the file open dialog", 0.0f),
            SWITCH(UI_ENABLE_KNOB_SCALE_ACTIONS_ID, NULL, "Enable knob scale mouse actions", 1.0f),
            ...
            SWITCH(UI_OVERRIDE_HYDROGEN_KITS_ID, NULL, "Override Hydrogen kits", 1.0f),
            SWITCH(UI_INVERT_VSCROLL_ID, NULL, "Invert global mouse vertical scroll behaviour", 0.0f),
            SWITCH(UI_GRAPH_DOT_INVERT_VSCROLL_ID, NULL, "Invert mouse vertical scroll behaviour for graph dot widget", 0.0f),
            SWITCH(UI_ZOOMABLE_SPECTRUM_GRAPH_ID, NULL, "Enables the automatic scaling mode of the frequency graph", 1.0f),
            COMBO(UI_FILTER_POINT_THICK_ID, NULL, "Thickness of the filter point", 1.0f, filter_point_thickness_modes),
            PATH(UI_DOCUMENTATION_PATH_ID, "Path to the local documentation installation"),
            SWITCH(UI_FILELIST_NAVIGAION_AUTOLOAD_ID, NULL, "Automatically load files when navigating over file list", 0.0f),
            SWITCH(UI_TAKE_INST_NAME_FROM_FILE_ID, NULL, "Take instrument name from the name of loaded file", 0.0f),
            PORTS_END
        };
