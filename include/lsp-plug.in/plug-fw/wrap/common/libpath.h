...
#ifndef LSP_PLUG_IN_PLUG_FW_WRAP_COMMON_LIBPATH_H_
#define LSP_PLUG_IN_PLUG_FW_WRAP_COMMON_LIBPATH_H_
...
namespace lsp
{
    ...
    static char *getlibpath_skip_space(char *p, char *end)
    {
        while (p < end)
        {
            if ((*p != ' ') && (*p != '\t'))
    ...
    static char *getlibpath_skip_field(char *p, char *end)
    {
    ...
    static char *getlibpath_trim_file(char *p, char *end)
    {
    ...
    static bool getlibpath_check_presence(const char **paths, const char *path)
    {
        if (paths == NULL)
            return false;
        while (*paths != NULL)
    ...
    static bool getlibpath_add_path(getlibpath_path_t *res, const char *path, const char **exclude)
    {
    ...
    static ssize_t getlibpath_getline(char **line, getlibpath_buf_t *buf, FILE *fd)
    {
     ...
     */
    static bool getlibpath_proc(char ***paths, const char **exclude)
    {
    ...
    static bool getlibpath_procstat(char ***paths, const char **exclude)
    {
        char cmd[80];
        pid_t pid = getpid();
     ...
     */
    static void free_library_paths(char **paths)
    {
        if (paths == NULL)
            return;
    ...
    /**
     * Get the path of the self shared object file
     * @return pointer to string containing self path (should be free()'d)
     */
    static char *get_library_path()
    {
        Dl_info dli;
